...
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | Yes for model output | DeepSeek API key used by the backend. Never expose it in browser JS. |
...
- Text PDFs and `.docx` files are parsed by the backend.
- Image OCR requires Tesseract. The Dockerfile installs English, Simplified Chinese, and Japanese OCR data.
