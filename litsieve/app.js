...
function scoreSegment(segment, paper, model) {
  ...
  if (!paper.text && (paper.type === "image" || paper.source === "后端解析")) score += 2;
...
function buildUsableScope(paper, status, best, segments) {
  if (!paper.text) return "该材料需要后端先完成正文抽取和 OCR，随后输出内容概述、命中章节和可用性判断。";
...
function buildLimitations(paper, status, best, hasReadableText) {
  if (!hasReadableText) return "后端需要完成扫描件、图片或图片型 PDF 的 OCR，并返回内容概述、命中章节和可引用页码。";
